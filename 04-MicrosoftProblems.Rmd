...
In this section I want to follow the thought process of the conservative (dare I say lazy) IT Manager and demonstrate that there is no way you are going to achieve Mass Customisation with Microsoft Software. It is in the next Section we will get our first look at R Studio and see how things should be done! But lets get some ammunition first.
...
Microsoft's product strategy is not the same for each software module. On the one side, Microsoft offers its Office Products (eg. Excel, Word, Powerpoint, Access) using a Low Cost strategy to ensure mass market appeal and affordability. On the other side, Microsoft offers a suite of Advanced Analytics Products (eg. SQL Management Studio, Analysis Services, Reporting Services...) using a Focus Strategy to deliver enhanced features but at significant additional cost to the user.

So if a customer needs an Excel based tool with functionality to analyse data in an online transactional database, both Low Cost and High Cost modules must be purchased. If we analyse this approach using the trade-off curves of the previous section it is obviously not mass customisation:

(ref:Microsoft1) Performance trade-offs using Microsoft

```{r,echo=FALSE, fig.cap='(ref:Microsoft1)'}
...
When you click download, you'll get a zip file containing how-to documentation and two Microsoft Excel files containing the tool. 
...
As with all calculation engines, Fitch's tool requires data in a specific format. If you look at the figures xx xx, you will see a number of data validation and certification steps. This is actually the reason why a pop-up wizard was required. 
...
I mentioned above that there are two key models in the tool. The "sustainable home price model" calculates property resale values. In the figure below you'll see an example calculation:
...
In pictures (see figure xxx), we have the needs of the customer falling beyond the capabilities of the modular system. Consequently as with traditional Operations Strategy, significant cost is involved to customise the production of the tool. This is not Mass Customisation at work!!
